...
#define BLINKING_3s 60 // 3/0.05 = 60 colpi di clock per portare i 3 sec 
#define TOT_TIME 144 // totale tempo di spostamento 7.2sec/0.05 = 144
#define TIME_MOVING 0xABA9500// 7.2sec * 25e6 = 
#define INACTIVITY_TIME 1200 // 60/0.05 = 1200
...
volatile int down = 0; // variabile per la pressione dei pulsanti KEY1 e KEY2
...
{					
	...
	if ((LPC_GPIO1->FIOPIN & (1<<25)) == 0 && reserved == 0) { // attivo il joystick prima dell'utilizzo
		enable = 1;
		// eventualmente spengo gli ALARM LED
		LED_Off(1); // spengo gli alarm LED 
		LED_Off(3); // spengo gli alarm LED
	}
	else if((LPC_GPIO1->FIOPIN & (1<<26)) == 0 && reserved == 0 && arrived <= 0 && enable == 1 && elevator_floor!=0){	
		/* Joystick DOWN */
		/*
	...
	else if((LPC_GPIO1->FIOPIN & (1<<29)) == 0 && reserved == 0 && arrived <= 0 && enable == 1 && elevator_floor != 1){	
		/* Joytick UP*/
		/*
		...
		*/
		...
		if(selectUP > 0){
				...
				if ( movement == 1 && tot_time_movement >= TOT_TIME){
					...
					LED_Off(2); // spengo i RESERVED LED
					// 3 seconds at 5 Hz --> 5 Hz = 0.2sec -> 0.2 * 25e6 = 0x4C4B40 
			...
			if ( arrived <= 0 && reserved == 0){
				/*
					- arrived == 0 non sono arrivato a destinazione 
					- reserved == 0 non ho nessuna prenotazione dell'ascensore
			...
			if(reserved != 0){
				init_timer(0, BLINKING_MOVING); // blinking in movimento
				enable_timer(0);
			}				
	}
...
// ---> BUTTON MANAGEMENT <---
	/* button management */
		if(down!=0){ 
		if((LPC_GPIO2->FIOPIN & (1<<11)) == 0 && movement == 0 && reserved == 0 ){	/* KEY1 pressed _ FLOOR 1 - ascensore libero e non in movimento tramite joystick e non riservato*/
			down++;				
			switch(down){
				case 2:
					// here my code
					...
					LED_On(0);
					LED_On(2);
					reserved = 0; // in questa maniera sono tranquillo che posso usare il joystick e sono allo stesso piano dell'ascensore
		...
		else if((LPC_GPIO2->FIOPIN & (1<<12)) == 0 && movement == 0 && reserved == 0 ){ /* KEY2 pressed _ FLOOR 0 - ascensore libero e non in movimento tramite joystick*/
			down++;				
			switch(down){
				case 2:
					// here my code
